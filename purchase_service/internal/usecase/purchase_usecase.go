...
func (uc *PurchaseUsecase) ListTicketsByUser(userID int64) ([]model.Purchase, error) {
...
func (uc *PurchaseUsecase) UpdatePurchase(purchaseID, userID int64, newNumbers []int32) error {
...
func (uc *PurchaseUsecase) DeletePurchase(purchaseID, userID int64) error {
	err := uc.db.Transaction(func(tx *gorm.DB) error {
		var exists bool
		err := tx.Raw("SELECT EXISTS (SELECT 1 FROM purchases WHERE id = ? AND user_id = ?)", purchaseID, userID).Scan(&exists).Error
		if err != nil {
